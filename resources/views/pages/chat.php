...
<div class="gradient__bg h-screen w-screen">
  <div id="app" class="w-screen h-screen flex flex-col items-center justify-between">
      ...
      <div
        ...
        <div class="w-full p-3 flex justify-between mt-4">
          ...
          foreach ($content as $key => $value) {
            ...
            $hover = ($key == "examples") ? 'hover:bg-[#021427]' : '';
            $arrow = ($key == "examples") ? '&rarr;':'';
            echo '<div class="w-1/3">
            <h3 class="text-center text-xl text-[#FF8A71] mt-2 mb-6 capitalize">' . $key . '</h3>
            <div class="w-4/5 mx-auto bg-[#031B34] py-3 my-3 rounded-lg ' . $cursor . ' ' . $hover . '" id ="' . $idPrefix . '1">
              <p class="text-gray-200 text-center w-full px-3 mx-auto text-sm">"' . $value[0] . '"'.$arrow.'</p>
            </div>
            <div class="w-4/5 mx-auto bg-[#031B34] px-2 py-3 my-3 rounded-lg ' . $cursor . ' ' . $hover . '" id="' . $idPrefix . '2">
              <p class="text-gray-200 text-center w-full px-3 mx-auto text-sm">"' . $value[1] . '"'.$arrow.'</p>
            </div>
            <div class="w-4/5 mx-auto bg-[#031B34] px-2 py-3 my-3 rounded-lg ' . $cursor . ' ' . $hover . '" id="' . $idPrefix . '3">
    ...
    <form class="w-11/12 bg-[#021427] rounded-md my-3 p-0.5 drop-shadow-2xl relative z-20" id="form">
      <div class="flex items-end rounded-md bg-gray-50 dark:bg-[#021427] py-0.5 z-10">
        <textarea id="chat" rows="1" name="prompt"
          class="block overflow-y-auto resize-none w-full p-2.5 text-md text-gray-700 bg-white rounded-md focus:ring-blue-500 focus:border-blue-500 dark:bg-[#021427] dark:border-0 dark:placeholder-gray-400 dark:text-white dark:focus:ring-0 dark:focus:border-0 flex-grow"
        ...
        <button type="submit"
          ...
          <svg aria-hidden="true" class="w-5 h-5 rotate-90 fill-gray-400" fill="currentColor" viewBox="0 0 20 20"
